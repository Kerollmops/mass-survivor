use bevy::{time::Stopwatch, prelude::Component};
...
impl TimerComponent {
    ...
    pub fn new(duration: Duration, repeating: bool) -> Self {
        Self { duration, repeating, ..Default::default() }
    }
    ...
    pub fn from_seconds(duration: f32, repeating: bool) -> Self {
        Self { duration: Duration::from_secs_f32(duration), repeating, ..Default::default() }
    }
...
mod tests {
    ...
    fn non_repeating_timer() {
        let mut t = TimerComponent::from_seconds(10.0, false);
    ...
    #[test]
    fn repeating_timer() {
        let mut t = TimerComponent::from_seconds(2.0, true);
        // Tick once, check all attributes
    ...
    fn times_finished_repeating() {
        let mut t = TimerComponent::from_seconds(1.0, true);
        assert_eq!(t.times_finished_this_tick(), 0);
    ...
    fn times_finished_this_tick() {
        let mut t = TimerComponent::from_seconds(1.0, false);
        assert_eq!(t.times_finished_this_tick(), 0);
        t.tick(Duration::from_secs_f32(1.5));
        assert_eq!(t.times_finished_this_tick(), 1);
    ...
    fn times_finished_this_tick_precise() {
        let mut t = TimerComponent::from_seconds(0.01, true);
    ...
    #[test]
    fn paused_repeating() {
        let mut t = TimerComponent::from_seconds(10.0, true);
